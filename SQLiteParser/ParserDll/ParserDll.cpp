...
int SyntaxCheck(int nType,LPCTSTR szSql, int iRow, int iCol,  int* iHintCount, SyntaxCheckStruct2** pSyntaxCheck,
				int* iMatchCount, BlockMatchStruct2** pBlockMatch, int* iAliasCount, QueryAlias2** pQueryAlias)
{
	...
	int rslt=SqliteParserHelper::SqlScriptCheck(nType,sSql,"",iRow,iCol, vHints, vMatchs, vAutocomplete, vAlias);
	//get syntax check information
	vector<SyntaxCheckStruct>::iterator itor;

	*iHintCount=vHints.size();

	*pSyntaxCheck=(SyntaxCheckStruct2*)malloc(vHints.size()*sizeof(SyntaxCheckStruct2)+1);
    SyntaxCheckStruct2* pTop=*pSyntaxCheck;

	for (itor=vHints.begin();itor!=vHints.end();itor++)
	{
		pTop->szHint=GetPChar((*itor).szHint);
		pTop->iRow=(*itor).iRow;
		pTop->iBeginCol=(*itor).iBeginCol;
		pTop->iEndCol=(*itor).iEndCol;

		pTop++;
	}

	//get blockmatch information
	*iMatchCount=vMatchs.size();
	*pBlockMatch=(BlockMatchStruct2*)malloc(vMatchs.size()*sizeof(BlockMatchStruct2)+1);
	BlockMatchStruct2* pTop2=*pBlockMatch;
	vector<BlockMatchStruct>::iterator itor2;
	for (itor2=vMatchs.begin();itor2!=vMatchs.end();itor2++)
	{
        pTop2->iRow=(*itor2).iRow;
		pTop2->iKind=(*itor2).iKind;
		pTop2->iBeginCol=(*itor2).iBeginCol;
		pTop2->iEndCol=(*itor2).iEndCol;

		pTop2++;
	}

	//get alias table information
	*iAliasCount=vAlias.size();
	*pQueryAlias=(QueryAlias2*)malloc(vAlias.size()*sizeof(QueryAlias2)+1);
	QueryAlias2* pTop3=*pQueryAlias;
	vector<QueryAlias>::iterator itor3;
	for (itor3=vAlias.begin();itor3!=vAlias.end();itor3++)
	{
		pTop3->iBeginCol=(*itor3).iBeginCol;
		pTop3->iBeginRow=(*itor3).iBeginRow;
		pTop3->iEndRow=(*itor3).iEndRow;
		pTop3->iEndCol=(*itor3).iEndCol;

		vector<AliasTable>::iterator aliasItor;

		string szAlias="";
		for (aliasItor=(*itor3).vAlias.begin();aliasItor!=(*itor3).vAlias.end();aliasItor++)
		{
			szAlias=szAlias+(*aliasItor).szAlias+"="+(*aliasItor).szTable+"\n";
		}

		pTop3->pAlias=GetPChar(szAlias);
