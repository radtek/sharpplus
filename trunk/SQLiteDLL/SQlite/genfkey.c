...
**
** $Id: genfkey.c,v 1.3 2009/02/25 15:43:57 danielk1977 dead $
*/
...
int sqlite3_genfkey(sqlite3 *db,const char *zDb)
//int sqlite3_genfkey(sqlite3 *db)
{
    GenfkeyCmd cmd;
	cmd.db = db;
	cmd.isExec = 1;
	cmd.isIgnoreErrors = 1;
	cmd.isNoDrop = 0;
	cmd.nErr = 0;
	genfkey_create_triggers(db, zDb, (void *)&cmd, genfkeyCmdCb);
//	genfkey_create_triggers(db, "main", (void *)&cmd, genfkeyCmdCb);
